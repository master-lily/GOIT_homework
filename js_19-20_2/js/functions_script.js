...
var data = [
    ...
    var skillsResult = _(data).map('skills').flattenDeep().sortBy().sortedUniq().value();
    console.log('Массив скилов: ', skillsResult);


    //2. Массив имен (поле name) людей, отсортированных в зависимости от количества их друзей (friends);
    var nameResult = _(data).map('name').sortBy('friends').value();
    console.log('Массив имен: ', nameResult);



    //3. Массив всех друзей всех пользователей, не должно быть повторяющихся людей
    var friendsResult = _(data).map('friends').flatten().map('name').uniq(data).value();
    console.log('Массив друзей: ', friendsResult);
